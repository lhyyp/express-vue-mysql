<template>
	<div  class="login-wrap">
        ...
        <form class="ms-login">
            ...
            <div>
            	<input type="password"  class='password' placeholder='请输入密码'  v-model='data.password'  @keyup.enter='login()'>
            </div>
            <div>
            	 <i-button type="success"  class="login-btn" @click='login()'>登录</i-button>
...
<script>
	...
	export default{
		...
		methods:{
			login(){
				var _this=this;
                axios.post('/api/admin/common/login',{data:_this.data}).then(
                	function(res){
